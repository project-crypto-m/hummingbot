...
from hummingbot.connector.exchange.swaphere.swaphere_constants import (
    ...
    SWAPHERE_PRODUCT_TRADES_PATH,
    SWAPHERE_PRODUCTS_PATH,
)
from hummingbot.connector.exchange.swaphere.swaphere_utils import get_new_client_order_id

TradingRule = getattr(importlib.import_module("hummingbot.connector.trading_rule"), "TradingRule")
ExchangeBase = getattr(importlib.import_module("hummingbot.connector.exchange_base"), "ExchangeBase")
OrderType = getattr(importlib.import_module("hummingbot.core.data_type.common"), "OrderType")
TradeType = getattr(importlib.import_module("hummingbot.core.data_type.common"), "TradeType")
InFlightOrder = getattr(importlib.import_module("hummingbot.core.data_type.in_flight_order"), "InFlightOrder")
OrderState = getattr(importlib.import_module("hummingbot.core.data_type.in_flight_order"), "OrderState")
OrderUpdate = getattr(importlib.import_module("hummingbot.core.data_type.in_flight_order"), "OrderUpdate")
TradeUpdate = getattr(importlib.import_module("hummingbot.core.data_type.in_flight_order"), "TradeUpdate")
TradeFeeBase = getattr(importlib.import_module("hummingbot.core.data_type.trade_fee"), "TradeFeeBase")
AddedToCostTradeFee = getattr(importlib.import_module("hummingbot.core.data_type.trade_fee"), "AddedToCostTradeFee")
TokenAmount = getattr(importlib.import_module("hummingbot.core.data_type.trade_fee"), "TokenAmount")
CancellationResult = getattr(
    importlib.import_module("hummingbot.core.data_type.cancellation_result"), "CancellationResult"
)
RESTMethod = getattr(importlib.import_module("hummingbot.core.web_assistant.connections.data_types"), "RESTMethod")
RESTAssistant = getattr(importlib.import_module("hummingbot.core.web_assistant.rest_assistant"), "RESTAssistant")
WebAssistantsFactory = getattr(
    importlib.import_module("hummingbot.core.web_assistant.web_assistants_factory"), "WebAssistantsFactory"
)
OrderBook = getattr(importlib.import_module("hummingbot.core.data_type.order_book"), "OrderBook")
safe_gather = getattr(importlib.import_module("hummingbot.core.utils.async_utils"), "safe_gather")
safe_ensure_future = getattr(importlib.import_module("hummingbot.core.utils.async_utils"), "safe_ensure_future")
AsyncThrottler = getattr(importlib.import_module("hummingbot.core.api_throttler.async_throttler"), "AsyncThrottler")
RATE_LIMITS = getattr(
    importlib.import_module("hummingbot.connector.exchange.swaphere.swaphere_constants"), "RATE_LIMITS"
)


class SwaphereExchange(ExchangeBase):
    """
    SwaphereExchange connects with the SwapHere exchange using private key authentication.
    This exchange uses the Ethereum wallet to sign orders.
    """

    def __init__(
        self,
        client_config_map=None,
        swaphere_private_key: str = None,
        trading_pairs: List[str] = None,
        trading_required: bool = True,
        **kwargs,
    ):
        """
        Initialize SwapHere exchange.
        :param client_config_map: Client configuration map
        :param swaphere_private_key: Ethereum private key
        :param trading_pairs: List of trading pairs to track
        :param trading_required: Whether actual trading is required
        """
        # Handle the case when we're called from the proxy in __init__.py
        if client_config_map is None and kwargs.get("client_config_map") is not None:
            client_config_map = kwargs.get("client_config_map")

        super().__init__(client_config_map)

        # Extract private key from API keys if needed (for connect command compatibility)
        # Since we set USE_ETHEREUM_WALLET = False, the private key will be passed as api_key
        if swaphere_private_key is None and "swaphere_private_key" in kwargs:
            swaphere_private_key = kwargs["swaphere_private_key"]

        self._private_key = swaphere_private_key
        self._trading_pairs = trading_pairs or []
        self._trading_required = trading_required

        self._auth = SwaphereAuth(self._private_key)
        self._throttler = AsyncThrottler(rate_limits=RATE_LIMITS)
        self._web_assistants_factory = WebAssistantsFactory(auth=self._auth, throttler=self._throttler)
    ...
    async def _update_trading_rules(self):
        ...
        for product in products_info:
            try:
                ...
                quote_increment = Decimal(product.get("quoteIncrement", "0.00000001"))  # noqa: F841
                base_scale = product.get("baseScale", 8)
                quote_scale = product.get("quoteScale", 8)  # noqa: F841
    ...
    async def place_order(
        ...
        base_token_info = self._tokens.get(
            base_token, {"address": f"0x{base_token}000000000000000000000000000000000000", "decimal": 18}
        )
        quote_token_info = self._tokens.get(
            quote_token, {"address": f"0x{quote_token}00000000000000000000000000000000000", "decimal": 18}
        )
        ...
        data = {"intent": intent, "type": ORDER_TYPE_MAP.get(order_type)}
    ...
    async def _api_request(
        ...
        resp = await self._rest_assistant.call(
            method=method, url=full_url, params=params, data=json.dumps(data) if data else None, headers=request_headers
        )
    ...
    async def cancel_order(self, client_order_id: str) -> Dict[str, Any]:
        ...
        response = await self._api_request(method=RESTMethod.DELETE, path=path)
